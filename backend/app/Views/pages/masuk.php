<div class="col">
    <div class="row">
        <div class="container col-5">
            <form class="p-5">
                <div class="text-center">
                    <div class="d-flex justify-content-center">
                        <img src="data:image/png;base64," style="height: 35px;" class="img-fluid" alt="">
                        <h1 class="fs-3">&nbsp; SIMS PPOB</h1>
                    </div>
                    <div class="d-flex justify-content-center py-3">
                        <h3 class="col-6">Masuk atau buat akun untuk memulai</h3>
                    </div>
                </div>
                <div class="form-floating mb-3">
                    <input type="email" class="form-control" id="email" placeholder="email">
                    <label for="email"><i class="fa-solid fa-at"></i>&nbsp; masukkan email Anda</label>
                </div>

                <div class="form-floating mb-3">
                    <input type="password" class="form-control" id="floatingPassword" placeholder="Password">
                    <label for="floatingPassword"><i class="fa-solid fa-lock"></i>&nbsp; masukkan password Anda</label>
                </div>
                <!-- <div class="checkbox mb-3">
                    <label>
                        <input type="checkbox" value="remember-me"> Remember me
                    </label>
                </div> -->
                <button class="w-100 btn btn-lg btn-danger" type="submit">Masuk</button>
                <div class="d-flex justify-content-center mt-4">
                    <small class="text-body-secondary">Belum punya akun? registrasi <a href="/" class="fw-bold text-decoration-none text-danger">di sini</a></small>
                </div>
            </form>
        </div>
        <div class="col-6">
            <img src="img/image1.png" class="img-fluid" alt="">
        </div>
    </div>
</div>